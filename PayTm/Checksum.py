...
def generate_checksum(param_dict, merchant_key, salt=None):
    params_string = _get_param_string_(param_dict)
    salt = salt if salt else _id_generator_(4)
...
def verify_checksum(param_dict, merchant_key, checksum):
    ...
    # Get salt
    paytm_hash = _decode_(checksum, IV, merchant_key)
    salt = paytm_hash[-4:]
...
def _id_generator_(size=6, chars=string.ascii_uppercase + string.digits + string.ascii_lowercase):
    return ''.join(random.choice(chars) for _ in range(size))


def _get_param_string_(params):
    params_string = []
...
_pad_ = lambda s: s + (BLOCK_SIZE - len(s) % BLOCK_SIZE) * chr(BLOCK_SIZE - len(s) % BLOCK_SIZE)
